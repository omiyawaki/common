...
def fit1110(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/3,
        ...
        50,
        0]
    bounds=([np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(LH)),-0.0001,0,-0.0001],
            [np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(LH)),0.0001,np.inf,0.0001])
    ...
    return {'type':'1110','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[Xnew,Ynew],'xc':xc,'yp':Y2}
...
def fit1100(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/3,
        ...
        0,
        50]
    bounds=([np.min(np.array(SM)),np.min(np.array(LH)),-0.0001,0],
            [np.max(np.array(SM)),np.max(np.array(LH)),0.0001,np.inf])
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=np.inf
    return {'type':'1100','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0110(data):
    ...
    p0=[SM.max()-(SM.max()-SM.min())/3,
        ...
        50,
        0]
    bounds=([np.min(np.array(SM)),np.min(np.array(LH)),0,-0.0001],
            [np.max(np.array(SM)),np.max(np.array(LH)),np.inf,0.0001])
...
def fit0100(data):
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=np.inf
    return {'type':'0100','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0010(data):
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=np.nan
    return {'type':'0010','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0011(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/3,
        ...
        0,
    -50]
    bounds=([np.min(np.array(SM)),np.min(np.array(LH)),-0.0001,-np.inf],
            [np.max(np.array(SM)),np.max(np.array(LH)),0.0001,0])
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=-np.inf
    return {'type':'0011','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0001(data):
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=-np.inf
    return {'type':'0001','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0101(data):
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=res[0][0]
    return {'type':'0101','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[X,Y],'xc':xc,'yp':Y2}
...
def fit0111(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/3,
        ...
        0,
        -50]
    bounds=([np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(LH)),0,-0.0001,-np.inf],
            [np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(LH)),np.inf,0.0001,0])
    res= optimize.curve_fit(piecewise3sg_linear, np.array(SM), np.array(LH),p0=p0,bounds=bounds,full_output=True,maxfev=1e10)
    X=np.r_[SM.min(),res[0][0],res[0][1],SM.max()]
    Y=np.r_[res[0][3]*(SM.min()-res[0][0])+res[0][2],
           res[0][2],
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=res[0][0]
    return {'type':'0111','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[Xnew,Ynew],'xc':xc,'yp':Y2}
...
def fit1101(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/3,
        ...
        50,
        -50]
    bounds=([np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(LH)),-0.0001,0,-np.inf],
            [np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(LH)),0.0001,np.inf,0])
    res= optimize.curve_fit(piecewise3sg_linear, np.array(SM), np.array(LH),p0=p0,bounds=bounds,full_output=True,maxfev=1e10)
    X=np.r_[SM.min(),res[0][0],res[0][1],SM.max()]
    Y=np.r_[res[0][3]*(SM.min()-res[0][0])+res[0][2],
           res[0][2],
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=res[0][1]
    return {'type':'1101','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[Xnew,Ynew],'xc':xc,'yp':Y2}
...
def fit1111(data):
    ...
    p0=[SM.min()+(SM.max()-SM.min())/4,
        ...
        0,
        -50]
    bounds=([np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(LH)),-0.0001,0,-0.0001,-np.inf],
            [np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(LH)),0.0001,np.inf,0.0001,0])
    ...
    if "is satisfied" in res[3] or "are satisfied" in res[3]:
        ...
        flag=0
    xc=res[0][1]
    return {'type':'1111','flag1':res[4],'flag2':flag,'p':res[0],'AIC':AIC,'BIC':BIC,'line':[Xnew,Ynew],'xc':xc,'yp':Y2}
